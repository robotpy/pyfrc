...
def check_function(parent, fn, errors):
    ...
    if doc is None:
        ...
        args, varargs, keywords, defaults, _, _, annotations = inspect.getfullargspec(fn)
        ...
        if len(params) != len(args):
            ...
            for param, arg in zip(params, args):
                if param != arg:
                    print_fn_err(
                        "Param '%s' is out of order, does not match param '%s' in docstring for" % (param, arg), parent,
                        fn, errors)
